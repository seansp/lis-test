...
param(
	...
	Write-Host ">> Create-CredentialObject ( $User, $Password ) : $([DateTime]::Now)"
...
{
	Write-Host ">> Generate-PDKFile ( $shielding_type, $shareName ) : $([DateTime]::Now)"
...
{
	Write-Host ">> Upload_File ( $fileName, $share_name ) : $([DateTime]::Now)"
...
{
	Write-Host ">> Copy_template_from_share ( $share_name ) : $([DateTime]::Now)"
...
{
	Write-Host ">> CreateVM ( $dep_vhd ) : $([DateTime]::Now)"
...
{
	Write-Host ">> Modify_VSC ( $sshKey, $dep_IP, $shareName ) : $([DateTime]::Now)"
	# Mount the template
...
{
	Write-Host ">> Modify_boot_partition ( $sshKey, $dep_IP, $sharedName ) : $([DateTime]::Now)"
...
{
	Write-Host ">> Modify_root_partition ( $sshKey, $dep_IP, $shareName ) : $([DateTime]::Now)"
...
{	
	Write-Host ">> Copy_Files_to_GH ( $GH_IP, $share_path, $gh_creds, $share_creds ) : $([DateTime]::Now)"
...
{
	Write-Host ">> Provision_VM ( $GH_IP, $gh_creds, $change_fsk ) : $([DateTime]::Now)"
...
{
	Write-Host ">> Get_VM_ipv4 ( $GH_IP, $gh_creds ) : $([DateTime]::Now)"
...
{
	Write-Host ">> Verify_provisioned_VM ( $vm_ipv4, $sshKey ) : $([DateTime]::Now)"
...
{
	Write-Host ">> Clean_provisioned_VM ( $GH_IP, $gh_creds ) : $([DateTime]::Now)"
